...
elif [ -e administrator -a ! -d administrator/logs ]; then
...
if [ -d cache ]; then
    ...
    find cache/* -type d -exec chmod $CHMOD_OPTIONS 770 {} \; \
					-o -type d -exec chmod $CHMOD_OPTIONS 660 {} \;
    ...
    find administrator/cache/* -type d \( -exec chmod $CHMOD_OPTIONS 770 {} \; -exec chown $CHOWN_OPTIONS -R  $WWW_USER:$WWW_GROUP {} \; \) \
										-o \( -exec chmod $CHMOD_OPTIONS 660 {} \; -exec chown $CHOWN_OPTIONS -R  $WWW_USER:$WWW_GROUP {} \; \)
fi
#installation dir
...
if [ -d "components/com_easycreator" ]; then
    ...
    find components/com_easycreator ! -type f -exec chmod $CHMOD_OPTIONS 770 '{}' \; -o \
											-o -exec chmod $CHMOD_OPTIONS 660 '{}' \;
